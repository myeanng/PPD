...
//2015年4月23日13:47:45 投资列表查询	
	/**
	 *投资列表查询
	 *根据LoanID查找投资人
	 * 
	 */
	@RequestMapping(value="myAccount-investrecord" , method={RequestMethod.POST , RequestMethod.GET})
	public String investrecord(Integer loanid,HttpServletRequest request){
		Users user=(Users)request.getSession().getAttribute("user");
		Map map=aaaService.findLoanbyID(loanid, user.getUserid());
		Loan loan=(Loan)map.get("loan");
		Map hkmap=(HashMap)map.get("hkMap");
		List<Investrecord> investrecords=loan.getInvestrecords();
		List<Repaymentplan> repaymentplans=loan.getRepaymentplans();
		request.getSession().setAttribute("loan",loan);
		request.getSession().setAttribute("hkMap",hkmap);
		request.getSession().setAttribute("investrecords",investrecords);
		request.getSession().setAttribute("repaymentplans",repaymentplans);
		
		System.out.println(loan.getLoanmoney());
		System.out.println(loan.getRate());
		System.out.println("---------------------------------");
		for(Investrecord invest : investrecords){
			System.out.println(invest.getUsers().getUsername());
			System.out.println(invest.getMoney());
			System.out.println(invest.getInvesttime());
		}
		System.out.println("---------------------------------");
		
		for (Repaymentplan re : repaymentplans) {
			System.out.println(re.getShouldtime());
		}
		return "myloanInvestList";
...
//2015年4月23日13:47:45 投资列表查询	
	 ...
	 * 
	 */
	@RequestMapping(value="myAccount-hk" , method={RequestMethod.POST , RequestMethod.GET})
	public String hk(Integer loanid,Integer period,HttpServletRequest request){
		Users user=(Users)request.getSession().getAttribute("user");
		List<Bill> bills=aaaService.hk(loanid, period, user.getUserid());
...
//业务方法
	...
	public String upuserPWD(String password,String newPassword,HttpServletRequest request){
		
		//转换成MD5字符
		password=MD5.encodePassword(password, "MD5");
		Users u=new Users();
		Users user=(Users)request.getSession().getAttribute("user");
		
		if(user.getLoginpassword().equals(password)&&password!=null){
			u.setUserid(user.getUserid());
			newPassword=MD5.encodePassword(newPassword, "MD5");
